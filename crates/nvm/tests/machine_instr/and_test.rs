...
fn test_and_8bit_reg_to_mem(mut machine: Machine) {
    // AND [BX + SI], AL
    machine.run_instruction(Instruction::And(Operand::Memory(MemAddress {
...
fn test_and_mem_to_8bit_reg(mut machine: Machine) {
    // AND AL, [BX + SI]
    machine.run_instruction(Instruction::And(Operand::Register(Register::AL), Operand::Memory(MemAddress {
        base: Some(Register::BX),
        index: Some(Register::SI),
        displacement: 0,
        displacement_size: 0
...
fn test_and_mem_to_16bit_reg(mut machine: Machine) {
    // AND AX, [BX + SI]
    machine.run_instruction(Instruction::And(Operand::Register(Register::AX), Operand::Memory(MemAddress {
        base: Some(Register::BX),
        index: Some(Register::SI),
        displacement: 0,
        displacement_size: 0
...
fn test_and_8bit_reg_to_8bit_reg(mut machine: Machine) {
    // AND AL, CL
    machine.run_instruction(Instruction::And(Operand::Register(Register::AL), Operand::Register(Register::CL)));
...
fn test_and_16bit_reg_to_16bit_reg(mut machine: Machine) {
    // AND AX, CX
    machine.run_instruction(Instruction::And(Operand::Register(Register::AX), Operand::Register(Register::CX)));
